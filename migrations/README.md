...
- **006–027 + the `0001`–`0004` prompt track** — applied **by hand via the Supabase
...
high-water number is bumped as migrations are added. (Current high-water: **027**.)
...
- `schema_migrations` is **blind** to everything applied by hand, so it under-reports
  what's live.
- A **fresh/rebuilt database** would NOT receive 006–027 by running the tracked
