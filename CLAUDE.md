...
- **Migration apply gate** — a migration file is committed as **DRAFT (pending apply)**. Claude applies it with `psql` (`FINANCIAL_DB_URL` / `MOOSII_DB_URL` in `.env`) **only after Mark's go for that batch**: FINANCIAL first — pre-check, migration, verification, per file, stop on any failure — then a report; MOOSII only after a second go, same steps, same report. Each project's apply is recorded in the reconciliation entry + high-water, in its own commit. If Mark applies by hand in the SQL editor instead, the entry flips only on his confirmation. Never apply without a go; never assume an apply from a file existing.
